...
public class TestAutomobiliProprietari {
	...
	public static void main(String[] args) {
		...
		try {
			...
			System.out
					.println("Nel DB di automobili ci sono " + automobileService.listAllAutomobili().size() + " voci.");
			...
			testUpdateAutomobile(automobileService, proprietarioService);
			System.out
					.println("Nel DB di automobili ci sono " + automobileService.listAllAutomobili().size() + " voci.");

			testContoProprietariDiAutoImmatricolateDal(proprietarioService, automobileService);
			System.out
					.println("Nel DB di automobili ci sono " + automobileService.listAllAutomobili().size() + " voci.");
			System.out.println(
					"Nel DB di proprietari ci sono " + proprietarioService.listAllProprietari().size() + " voci.");
	...
	private static void testInsertProprietario(ProprietarioService proprietarioService) throws Exception {
		...
		Proprietario nuovoProprietario = new Proprietario("Marco", "Verdi", "MRCVRD99C22H501M",
				new SimpleDateFormat("yyyy-mm-dd").parse("1999-03-22"));
	...
	private static void testUpdateProprietario(ProprietarioService proprietarioService) throws Exception {
		...
		if (proprietarioService.listAllProprietari().isEmpty())
			throw new RuntimeException("FAILED : database vuoto.");
		Proprietario proprietarioAggiornato = new Proprietario("Paolo", "Bianchi", "CF di Paolo",
				new SimpleDateFormat("yyyy-mm-dd").parse("2010-03-22"));
	...
	private static void testUpdateAutomobile(AutomobileService automobileService,
			ProprietarioService proprietarioService) throws Exception {
		...
		automobileAggiornata.setProprietario(proprietarioService.listAllProprietari().get(0));
	...
	private static void testContoProprietariDiAutoImmatricolateDal(ProprietarioService proprietarioService,
			AutomobileService automobileService) throws Exception {
		System.out.println("__inizio testContoProprietariDiAutoImmatricolateDal...");
		if (proprietarioService.listAllProprietari().isEmpty() || automobileService.listAllAutomobili().isEmpty())
			throw new RuntimeException("FAILED : database vuoto.");
		int dataImmatricolazioneDa = automobileService.listAllAutomobili().get(0).getAnnoImmatricolazione();
		Long result = proprietarioService.contoProprietariDiAutoImmatricolateDal(dataImmatricolazioneDa);
		if (result < 1)
			throw new RuntimeException("FAILED : la ricerca non ha dato i risultati attesi.");
		System.out.println("__fine testContoProprietariDiAutoImmatricolateDal : PASSED");
	}
	...
	private static void testTutteAutomobiliConCodiceFiscaleProprietarioIniziaCon(AutomobileService automobileService)
		...
		List<Automobile> result = automobileService
				.tutteAutomobiliConCodiceFiscaleProprietarioIniziaCon(inizioCodiceFiscale);
		if (result.isEmpty())
			throw new RuntimeException("FAILED : la ricerca non ha dato i risultati attesi.");
	...
	private static void testTutteLeAutoConErrori(AutomobileService automobileService) throws Exception {
		...
		if (automobileService.listAllAutomobili().isEmpty())
			throw new RuntimeException("FAILED : database vuoto.");
		List<Automobile> result = automobileService.tutteLeAutoConErrori();
		if (result.isEmpty())
			throw new RuntimeException("FAILED : la ricerca non ha dato i risultati attesi.");
